    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main>
        <section className="grid gap-8 px-8 py-12 sm:px-6 lg:grid-cols-[1.15fr_.85fr] lg:items-center">
          ...
          <div className="page-enter animation-delay-150 rounded-2xl border border-primary/25 bg-surface p-6 shadow-2xl shadow-primary/10">
...
function Feature({ icon: Icon, title, text }: { icon: LucideIcon; title: string; text: string }) {
  return <article className="card-enter rounded-xl border border-border bg-background p-6 transition-[transform,border-color] duration-300 hover:-translate-y-1 hover:border-primary/60"><Icon className="h-6 w-6 text-primary" /><h2 className="mt-4 text-lg font-semibold">{title}</h2><p className="mt-2 text-sm leading-6 text-text-secondary">{text}</p></article>;
}
